...
class Config:
    ...
    # HOJAS A PROCESAR - Solo estas hojas serán procesadas
    HOJAS_PROCESAR = [
        'acompañamiento',
        'acompanamiento',
        'complementario',
    ...
    MAPEO_COLUMNAS = {
        ...
        # Docentes
        'maestro_de_instrumento': 'docente',
        'docente_piano_acompanamiento': 'docente',
        'docente_piano_complementario': 'docente',
        'docente': 'docente',
        'profesor': 'docente',
    }
...
class ProcesadorAsignaciones:
    ...
    def es_hoja_procesar(self, sheet_name: str) -> bool:
        """Verifica si la hoja debe ser procesada"""
        sheet_normalizado = self.cleaner.normalizar_nombre_columna(sheet_name)
        
        for hoja_permitida in Config.HOJAS_PROCESAR:
            hoja_normalizada = self.cleaner.normalizar_nombre_columna(hoja_permitida)
            if hoja_normalizada in sheet_normalizado or sheet_normalizado in hoja_normalizada:
                return True
    ...
    def detectar_fila_encabezados(self, df: pd.DataFrame) -> int:
        ...
        for idx in range(min(10, len(df))):
            ...
            if any(keyword in texto_fila for keyword in ['apellidos', 'nombres', 'estudiante', 'agrupacion', 'instrumento']):
                return idx
        return 0
    ...
    def procesar_hoja(self, archivo_nombre: str, ruta_archivo: str, sheet_name: str) -> Optional[List[Dict]]:
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                # Identificar tipo de asignación según la hoja
                registro['tipo_asignacion'] = self.identificar_tipo_hoja(sheet_name)
    ...
    def identificar_tipo_hoja(self, sheet_name: str) -> str:
        """Identifica el tipo de hoja basado en su nombre"""
        sheet_lower = sheet_name.lower()
        
        if 'acompa' in sheet_lower:
            return 'Acompañamiento'
        elif 'complementario' in sheet_lower:
            return 'Complementario'
        elif 'agrupacion' in sheet_lower:
            return 'Agrupaciones'
        elif 'conj' in sheet_lower or 'conjunto' in sheet_lower:
            return 'Conjunto Instrumental'
        else:
            return 'Otro'
...
def main_agrupaciones():
    print("\n" + "="*70)
    print("🎵 ETL - EXTRACCIÓN DE HOJAS ESPECÍFICAS (JSON)")
    print("   Hojas procesadas: Acompañamiento, Complementario,")
    print("                     Agrupaciones, Conj. Inst.")
    print("="*70)
    ...
    for archivo in archivos_excel:
        ...
        try:
            ...
            for sheet_name in xls.sheet_names:
                if procesador.es_hoja_procesar(sheet_name):
                    hojas_procesadas.append(f"{archivo} - {sheet_name}")
                    registros = procesador.procesar_hoja(archivo, ruta_completa, sheet_name)
                    if registros:
                        todos_registros.extend(registros)
                else:
                    hojas_ignoradas.append(f"{archivo} - {sheet_name}")
                    logger.info(f"⏭️  Ignorando hoja: '{sheet_name}'")
    ...
    print("="*70)
    print(f"\n✅ Hojas procesadas ({len(hojas_procesadas)}):")
    for hoja in hojas_procesadas:
        print(f"   - {hoja}")
    
    print(f"\n⏭️  Hojas ignoradas ({len(hojas_ignoradas)}):")
    for hoja in hojas_ignoradas:
        print(f"   - {hoja}")
    ...
    if todos_registros:
        ...
        ruta_json = ruta_salida / f"ASIGNACIONES_FILTRADAS_{timestamp}.json"
        ...
        # Mostrar resumen por tipo de asignación
        resumen_tipos = {}
        for registro in todos_registros:
            tipo = registro.get('tipo_asignacion', 'Sin tipo')
            resumen_tipos[tipo] = resumen_tipos.get(tipo, 0) + 1
        
        print(f"\n📋 Registros por tipo de asignación:")
        ...
        ruta_csv = ruta_salida / f"ASIGNACIONES_FILTRADAS_{timestamp}.csv"
        df_export.to_csv(ruta_csv, index=False, encoding='utf-8-sig', sep=';')
        print(f"💾 También exportado como CSV: {ruta_csv}")
        
    else:
        logger.error("❌ No se pudo extraer ningún registro de las hojas especificadas")
